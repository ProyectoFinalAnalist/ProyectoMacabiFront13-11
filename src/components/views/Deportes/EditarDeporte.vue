<template>
      ...
      <div class="col-md offset-md" v-if="deporte">
        <div class="card bg-light text-dark mb-5" style="width: 100%">
          <div v-if="deporte" class="card-body">
            <h4>
              Detalles del Deporte: <strong>{{ nombre }}</strong>
            </h4>
            <div>
              <p>
              ...
              <ul
                class="list-group mt-1 mb-4 text-center"
                style="font-size: x-large"
              >
                <li
                  class="list-group text-dark"
                  v-on:click="irA(categoria.idCategoria)"
                  :class="[
              ...
              <div class="justify-content-center d-flex">
                <button class="btn btn-success mb-3" @click="agregarCategoria">
                  Agregar Categoria
                </button>
              </div>
              <p>
              ...
              <div v-else>
                <h6 class="alert-sm mb-0 text-center p-2 m-2 rounded mb-3">
                  <strong
                    >Deporte {{ nombre }} no posee coordinadores
                    asigados</strong
                  >
                </h6>
              </div>
              ...
              <h5
                class="alert alert-danger alert-sm mb-0 text-center m-2 mb-3"
                v-if="message != null"
              >
  ...
  <div class="d-flex justify-content-center">
    <button class="btn btn-secondary">
      <router-link to="/deportes" class="nav-item nav-link" href="#"
        >Volver a Deportes</router-link
      >
    </button>
  </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div
            v-for="coordinador in coordinadoresModal"
            :key="coordinador.idUsuario"
          >
...
export default {
  setup() {
    ...
    onMounted(async () => {
      ...
      await categoriasStore.fetchElements(
        `${apiUrl}/categoria/${idDeporte}/deporte`
      );
      await usuariosStore.fetchElements(
        `${apiUrl}/deporte/${idDeporte}/coordinadores`
      );
    ...
    async function updateCoordinadores() {
      ...
      try {
        await axios.post(
          `${apiUrl}/deporte/${idDeporte}/agregarCoordinadores`,
          usuarios
        );
      } catch (error) {
