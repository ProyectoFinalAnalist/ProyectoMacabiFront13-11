<template>
	<div class="container mt-3">
		...
		<div class="d-flex justify-content-end input-group mb-3">
			<form class="d-flex" v-on:submit.prevent="buscar">
				...
				<div class="col-md-4"> <!-- Adjust the col-md-6 width to your preference -->
					<input type="text" class="form-control bg-light border-1 pr-5 mr-2" placeholder="Buscar..."
						v-model="busqueda">
				</div>
		...
		<table class="table table-striped table-bordered">
			...
			<tbody>
				<tr v-for="user in usuariosStore.getElementsList" :key="user.idUsuario">
					<td>{{ user.nombre }}</td>
					<td>{{ user.apellido }}</td>
					<td>{{ user.email }}</td>
					<td>{{ user.dni }}</td>
...
import { useElementStore } from "../../../utils/Store.ts";
import { onMounted } from "vue";

export default {
	setup() {
		const usuariosStore = useElementStore("usuario")()
		const busqueda = "";

		onMounted(() => {
			usuariosStore.fetchElements(apiUrl + '/usuario/getAll')
		})

		function buscar() {
			usuariosStore.filtrarXString(this.busqueda, document.getElementById("filtro").value);
		}
